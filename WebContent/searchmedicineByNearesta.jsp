...
// SQL query with ORDER BY clause to sort by distance
PreparedStatement pstmt = con.prepareStatement("SELECT pa.`Medicine Name`, pa.`Brand Name`, pa.`Price`, pa.`Expiry Date`, pa.`Gender`, pa.`Age`, pa.`Discount`, pl.`nameofmedical`, pl.`username`, pa.`Contact No.`, pl.`location`, " +
        "(3959 * acos(cos(radians(ul.`latitude`)) * cos(radians(pl.`latitude`)) * cos(radians(pl.`longitude`) - radians(ul.`longitude`)) + sin(radians(ul.`latitude`)) * sin(radians(pl.`latitude`)))) AS distance " +
        "FROM pharmaaccount pa " +
        "JOIN pharmalogin pl ON pa.`Name` = pl.`username` " +
        "JOIN userlogin ul ON ul.`username` = ? " +
        "WHERE pa.`Medicine Name` = ? " +
        "ORDER BY distance ASC"); // Sort by distance in ascending order

pstmt.setString(1, ShortDist.getUserName());
    ...
    <table border="3">
        ...
        <% while (rs.next()) { %>
            <tr>
                ...
                <td><%= rs.getString("nameofmedical") %></td>
                <td><%= rs.getString("username") %></td>
                <td><%= rs.getString("Contact No.") %></td>
                <td><%= rs.getString("location") %></td>
                <td><%= Math.round(rs.getDouble("distance") * 100.0) / 100.0 %>&nbsp;KM</td>
            </tr>
        <% } %>
    </table>
</center>
