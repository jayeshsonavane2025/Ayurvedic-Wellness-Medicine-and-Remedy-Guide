...
<body class="cbp-spmenu-push">
    <div class="main-content">
        ...
        <!-- //header-ends -->
        <!-- main content start-->
         <div id="page-wrapper" style="min-height: 1203px; background-image: url( https://c4.wallpaperflare.com/wallpaper/368/462/529/pills-medicine-capsules-wallpaper-preview.jpg);background-size: cover;">
...
PreparedStatement pstmt = con.prepareStatement("SELECT * FROM pharmaaccount WHERE `Gender` = ? and `Medicine Name`=? ORDER BY `Discount` DESC");
...
<center>
    <table border="3">
        ...
        <% while (rs.next()) { %>
            <tr>
                <%-- Retrieve data from the pharmaaccount table --%>
                ...
                <%-- Retrieve data from the pharmalogin table --%>
                <% 
                String pharmacistUsername = rs.getString(1);
                String medicalName = "";
                String location = "";
                double mlon = 0.0, mlat = 0.0, ulon = 0.0, ulat = 0.0, dist = 0.0;
                
                PreparedStatement pstmt3 = con.prepareStatement("SELECT * FROM pharmalogin WHERE `username` = ?  ");
                pstmt3.setString(1, pharmacistUsername);
                ResultSet rs3 = pstmt3.executeQuery();
                
                if (rs3.next()) {
                    medicalName = rs3.getString("nameofmedical");
                    location = rs3.getString("location");
                    mlon = rs3.getDouble("longitude");
                    mlat = rs3.getDouble("latitude");
                }
                rs3.close();
                
                String uname = ShortDist.getUserName();
                PreparedStatement pstmt2 = con.prepareStatement("SELECT * FROM userlogin WHERE `username` = ?");
                pstmt2.setString(1, uname);
                ResultSet rs2 = pstmt2.executeQuery();
                if (rs2.next()) {
                    ulon = rs2.getDouble("longitude");
                    ulat = rs2.getDouble("latitude");
                }
                rs2.close();
                
                // Calculate the distance using the ShortDist class
                dist = ShortDist.distance(ulat, mlat, ulon, mlon);
                %>
                
                <%-- Display retrieved data in the table --%>
                <td><%= medicalName %></td>
                <td><%= pharmacistUsername %></td>
